// Nous Portal：Portal 订阅与积分用量查询（由 CodexBar nous.js 1:1 移植）
import type { QuotaDetailRow, QuotaDetailSection, QuotaStrategy } from '@common/types'
import { recordOf, strOf } from '../util'

/**
 * Nous Portal（credential: token）：GET {PORTAL_URL}/api/oauth/account → 订阅/积分明细。
 * 令牌优先级：附加配置 NOUS_PORTAL_ACCESS_TOKEN > 提供商 API Key（脚本路径由宿主附加
 * Bearer 头，值为空时宿主不附加，这里手动构造并保持一致）。
 */
export const nousStrategy: QuotaStrategy = {
  meta: {
    id: 'nous',
    label: 'Nous Portal',
    builtin: true,
    credential: 'token',
    description: 'Portal 用量；附加配置 portalUrl 可自定义服务地址',
    settings: [
      { key: 'PORTAL_URL', title: 'Portal 地址', hint: 'Portal 服务地址；未配置时无法查询用量' },
      { key: 'NOUS_PORTAL_ACCESS_TOKEN', title: '访问令牌', type: 'secure', hint: '可选；缺省使用提供商 API Key' }
    ]
  },
  async fetch(ctx) {
    const token = strOf(ctx.config.NOUS_PORTAL_ACCESS_TOKEN) ?? (ctx.apiKey.trim() ? ctx.apiKey.trim() : null)
    const headers: Record<string, string> = token ? { Authorization: `Bearer ${token}` } : {}
    const portal = strOf(ctx.config.PORTAL_URL)
    const response = await ctx.http.get(`${portal}/api/oauth/account`, { timeoutSeconds: 15, headers })
    ...
    let decoded: unknown
    try {
      decoded = JSON.parse(String(response.bodyText))
    } catch {
    ...
    const subscriptionRows: QuotaDetailRow[] = []
    ...
    const creditRows: QuotaDetailRow[] = []
    ...
    return {
      primary:
        monthly !== undefined && monthly > 0 && remaining !== undefined
          ? {
              usedPercent: ctx.pct(Math.max(0, monthly - Math.max(0, remaining)), monthly),
              resetsAt: renewal ? renewal.getTime() : undefined
            }
          : undefined,
      details,
      identity: {
