import type { QuotaDetailRow, QuotaSnapshot, QuotaStrategy } from '@common/types'
import { recordOf, strOf } from '../util'

const KEY_URL = 'https://api.meta.ai/muse-code/key'

/**
 * Muse：POST /muse-code/key 查询订阅 5 小时与每周用量窗口（由 CodexBar muse.js 1:1 移植）。
 * 凭证须为 `muse login` 签发的 dca: 设备码；resets_at 为 epoch 秒（超界丢弃但保留其余数据）。
 */
export const museStrategy: QuotaStrategy = {
  meta: {
    id: 'muse',
    label: 'Muse',
    builtin: true,
  ...
  async fetch(ctx) {
    ...
    for (const key of ['require_payment', 'is_subs_active']) {
      ...
      if (value !== undefined && value !== null && typeof value !== 'boolean') return fail(key)
    }
